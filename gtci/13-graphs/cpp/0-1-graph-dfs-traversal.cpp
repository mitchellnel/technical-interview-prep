...
public:
    ...
    std::vector<int> dfs_traversal(int start_node) {
        ...
        while (!stack.empty()) {
            const int curr = stack.top(); stack.pop();
...
int main() {
    ...
    assert(g1.dfs_traversal(0) == std::vector<int>({0,2,1,3}));
    ...
    assert(g5.dfs_traversal(0) == std::vector<int>({0,2,5,1,4,3}));

    Graph g6({{0, {1,2,3,4}}, {1, {}}, {2, {}}, {3, {}}, {4, {}}});
    assert(g6.dfs_traversal(0) == std::vector<int>({0,4,3,2,1}));
